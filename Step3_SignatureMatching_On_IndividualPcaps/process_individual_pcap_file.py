...
for key, value in data.items():
	...
	for f in files:
		...
		# 
		# if f_split[0] == '770':
		if value['custom_fields']['_Product_id'] == f_split[0]:
			folder = os.listdir(f)
			# print(folder_files)
				...
				if ('.pcap' in files_in_folder) or ('.PCAP' in files_in_folder):
					...
					if '-' in name_split[1]:
						...
						if  get_activity_name[-2] == 'fail':
							...
							activity_names_holder.append(activity_name)
						else:
					...
					for activity in activities_lst:	
						...
						if activity == 'LOGIN' or activity == 'LOGIN-FAIL' or activity == 'LOGOUT':
							if value["custom_fields"][activity+'_req-method'] != 'REMAINING' and value["custom_fields"][activity+'_req-method'] != 'NA' :
								...
								if activity == 'LOGIN-FAIL':
									if activity_name == activity.lower():
										
										# print(f+'/'+files_in_folder)

										pcap_file = pyshark.FileCapture(f+'/'+files_in_folder)
										packets_dict = open_pcap_packets(pcap_file,f+'/'+files_in_folder)
								...
								if found_pcap_flag == False:
									not_found[activity] = 'Pcap not found'
							...
							if value["custom_fields"][activity+'_req-method'] != 'REMAINING' and value["custom_fields"][activity+'_req-method'] != 'NA':
								...
								if size > 1:
									...
									for i in range(size):
										...
										if activity_name == activity.lower() and (int(activity_count)) == i:
											# print("^^^^^^^^^^^^^^^^^^^^^^^^")
											# print(activity_name)
											# print(int(activity_count)-1)
											# print("^^^^^^^^^^^^^^^^^^^^^^^^")
											pcap_file = pyshark.FileCapture(f+'/'+files_in_folder)
											packets_dict = open_pcap_packets(pcap_file, f+'/'+files_in_folder)
											packets_dict = dict(sorted(packets_dict.items()))			
											...
											# print(inner_dict)
											if activity in inner_dict:
												inner_dict[activity].append({i : pattern_matching(packets_dict, other_values,activity, app_name)})
											else:
												inner_dict[activity] = [{i : pattern_matching(packets_dict, other_values,activity, app_name)}]
											# print(ls)
									...
									if activity_name == activity.lower():
										...
										packets_dict = open_pcap_packets(pcap_file, f+'/'+files_in_folder)
										packets_dict = dict(sorted(packets_dict.items()))	
										inner_dict[activity] = pattern_matching(packets_dict, value,activity,app_name)
									else:
										not_found[activity] = 'Pcap not found'

					outer_dict[key] = inner_dict
					not_found_writer[key] = not_found

			with open ('JAN_results_on_individualpcaps_100.json', 'w') as outfile:
				json.dump(outer_dict, outfile, indent=4)

			with open ('pcap_notfound.json', 'w') as outfile:
				json.dump(not_found_writer, outfile, indent=4)
